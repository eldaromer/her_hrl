...
class DDPG:
	def __init__(self, env, batch_size=256, max_memory=500000, tau=0.01, gamma=0.99, hidden_size=400, verbose=True):
		...
		self.memory = ReplayMemory(max_memory, self.env)
	...
	def train(self, num_ep):
		...
		for ep in progressbar.progressbar(range(num_ep), widgets=widgets):
			...
			for t in range(10000):
				...
				new_observation = new_observation['observation']
				
				# Store transition is replay max_memory
				self.memory.push(observation, action, reward, new_observation, done, goal, info)
